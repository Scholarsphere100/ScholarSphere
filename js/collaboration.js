...
export async function loadRequests(userId) {
  try {
    const snapshot = await db.collection("collaborationRequests").orderBy("createdAt", "desc").get()
    ...
    for (const doc of snapshot.docs) {
      ...
      try {
        ...
        // Active collaborations - only show accepted requests
  ...
  try {
    ...
    if (response === "accepted" && request.projectId) {
      ...
      await db.collection("notifications").add({
        ...
        message: `${user.displayName || "A researcher"} has accepted your collaboration request for "${request.projectTitle || "a project"}".`,
        read: false,
    ...
    // Handle rejected requests - THIS WAS MISSING!
    if (response === "rejected") {
      ...
      await db.collection("notifications").add({
        ...
        message: `${user.displayName || "A researcher"} has declined your collaboration request for "${request.projectTitle || "a project"}".`,
        read: false,
        createdAt: firebase.firestore.Timestamp.now(),
        projectId: request.projectId,
        requestId: requestId,
    ...
    alert(`Request ${response === "accepted" ? "accepted" : "declined"}`)
    return true
  } catch (error) {
...
if (typeof exports !== "undefined") {
  ...
  // Project Reports
  exports.generateProjectCompletionReport =
    typeof generateProjectCompletionReport !== "undefined" ? generateProjectCompletionReport : () => {}
  exports.groupMilestonesByProject =
    typeof groupMilestonesByProject !== "undefined" ? groupMilestonesByProject : () => {}
